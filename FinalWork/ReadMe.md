...
```*.csv``` - пример файла с данными

### Документация:

[Документация Doxygen](Doc\html\index.html) - полная документация по проекту, сгенерированная с помощью Doxygen.
...
- Температура должна быть в диапазоне от `-99` до `99` градусов
